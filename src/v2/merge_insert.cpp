...
namespace diskann {
 ...
 template<typename T, typename TagT>
 void MergeInsert<T, TagT>::showVectorNum(){
    if(_active_index == 0){
        diskann::cout << "mem_index 0 vector num is: " << _mem_index_0->get_num_points() <<std::endl;
    }
    else{
        diskann::cout << "mem_index 1 vector num is: " << _mem_index_1->get_num_points() <<std::endl;
    }

    diskann::cout << "disk_index vector num is: " << _disk_index->return_nd()<<std::endl;
 }
