...
std::atomic<int>        cur_vector_count(0);
...
void ShowMemoryStatus() {
  ...
  std::cout << "memory current time: " << current_time << " RSS : " << rss/1024
            << " MB" << std::endl;
...
void ShowMemoryStatus(double dataset_size) {
  int current_time = globalTimer.elapsed() / 1.0e6f - begin_time;
  ...
  std::cout << "memory current time: " << current_time << " RSS : " << (rss / 1024-dataset_size)
            << " MB" << std::endl;
  // char           dir[] = "/home/fresh/update_store/store_diskann_100m";
  // DIR*           dp;
  // struct dirent* entry;
...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS)
  ...
  if (calRecall) {
    ...
    //   "~/baselines/DiskANN_Baseline/data/query_results/argoverse2_" +
...
template<typename T, typename TagT>
void concurrent_insertion_kernel(T*                             data_load,
                                 diskann::MergeInsert<T, TagT>& sync_index,
                                 std::vector<TagT>& insert_vec, _s64 start,
                                 _s64 end, size_t aligned_dim) {
  diskann::Timer      timer;
  size_t              npts = insert_vec.size();  //(size_t)(end - start);
  std::vector<double> insert_latencies(npts, 0);

  std::cout << "Begin Insert, target: " << end << std::endl;
  cur_vector_count.store(start);
#pragma omp parallel for num_threads(NUM_INSERT_THREADS)
  for (_s64 i = start; i < end; i++) {
    while (pause_flag) {
    ...
    if (cur_vector_count.load() < end) {
      cur_vector_count.fetch_add(1);
    }
  ...
  std::cout << "Inserted " << (end - start) << " points in " << time_secs << "s"
            << std::endl;
...
void concurrent_update(
    const std::string& data_path, const unsigned L_mem, const unsigned R_mem,
    const float alpha_mem, const unsigned L_disk, const unsigned R_disk,
    const float alpha_disk, size_t dim, int step, int start_num, int end_num,
    bool streaming, int total_time, int time_unit, const size_t base_num,
    const unsigned num_pq_chunks, const unsigned nodes_to_cache,
    std::string& save_path, const std::string& query_file,
    std::string& truthset_file, const int recall_at, _u64 Lsearch,
    const unsigned beam_width, std::string& trace_file_prefix,
  ...
  T*     base_vec = NULL;
  size_t base_vec_num, base_vec_dim, base_vec_aligned_dim;
  diskann::load_aligned_bin<T>(data_path, base_vec, base_vec_num, base_vec_dim,
                               base_vec_aligned_dim);

  // assign id for each vector
  std::vector<unsigned> insert_vec_ids(base_vec_num);
  std::iota(insert_vec_ids.begin(), insert_vec_ids.end(), 0);

  // diskann::load_aligned_bin<T>(data_path.c_str(), data_load, num_points,
  // base_dim,
  //                              aligned_dim);

  // std::cout << "Loaded full data for driver: (" << num_points << "," <<
  // base_dim
  //           << ") vectors." << std::endl;
  ...
  diskann::MergeInsert<T, TagT> sync_index(
      ...
      metric, false, save_path);  // index object?
  ...
  std::cout << "Loading base split range " << std::endl;
  unsigned* base_range = NULL;
  size_t    base_range_num;
  diskann::load_range_bin<unsigned>(trace_file_prefix, base_range,
                                    base_range_num);
  ...
  for (int i = start_num; i <= end_num; i += batch) {
    std::cout << "Batch: " << i << " Total Batch : " << end_num << std::endl;

    unsigned base_split_pos = *(base_range + i);
    unsigned last_vec_num;
    if (i == start_num) {
      last_vec_num = *(base_range + i / 10) + 1;
    } else {
      last_vec_num = *(base_range + i - batch) + 1;
    }

    currentFileName = truthset_file + std::to_string(i);
    ...
    std::future<void> insert_future =
        std::async(std::launch::async, concurrent_insertion_kernel<T, TagT>,
                   base_vec, std::ref(sync_index), std::ref(insert_vec_ids),
                   last_vec_num, base_split_pos + 1, dim);
    ...
    if (streaming) {
      do {
        ...
        if (insert_status == std::future_status::deferred) {
          ...
          if (notSearch) {
            pause_threads();
            std::cout << "Epoch: " << i << "\n";
            sync_index.showVectorNum();
            sync_search_kernel(query, i, query_aligned_dim, recall_at, Lsearch,
                               sync_index, currentFileName, inactive_tags,
                               base_split_pos + 1, false, true);
            resume_threads();
            notSearch = false;
          }
        }
      ...
      int time_count = 0;
      int final_count = total_time / time_unit;

      do {
        insert_status = insert_future.wait_for(std::chrono::seconds(time_unit));
    ...
    if (i == end_num) {
      std::cout << "The recall of final index state:\n";
      ShowMemoryStatus();
      sync_search_kernel(query, i, query_aligned_dim, recall_at, Lsearch,
                         sync_index, currentFileName, inactive_tags,
                         base_split_pos + 1, false, true);
...
int main(int argc, char** argv) {
  ...
  int         dim = (int) std::atoi(argv[arg_no++]);
  int         step = (int) std::atoi(argv[arg_no++]);
  int         start_num = (int) std::atoi(argv[arg_no++]);
  int         end_num = (int) std::atoi(argv[arg_no++]);
  bool        streaming = true;

  if (std::string(argv[arg_no++]) == std::string("false"))
    streaming = false;

  int total_time = (int) std::atoi(argv[arg_no++]);
  int time_unit = (int) std::atoi(argv[arg_no++]);
  ...
  if (updateIndex) {
    if (std::string(argv[1]) == std::string("int8")) {
      ...
      concurrent_update<float, unsigned>(
          full_data_path, L_mem, R_mem, alpha_mem, L_disk, R_disk, alpha_disk,
          dim, step, start_num, end_num, streaming, total_time, time_unit,
          num_start, num_pq_chunks, nodes_to_cache, save_path, query_file,
