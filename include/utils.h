...
namespace diskann {
  ...
  template<typename T>
  inline void load_range_bin_impl(std::basic_istream<char>& reader, T*& data,
                                  size_t& npts, size_t offset = 0) {
    int npts_i32;
    reader.seekg(offset, reader.beg);
    reader.read((char*) &npts_i32, sizeof(int));
    ...
    diskann::cout << "Metadata: #pts = " << npts << "..." << std::flush;
    size_t allocSize = npts * sizeof(T);
    alloc_aligned(((void**) &data), allocSize, 8 * sizeof(T));

    for (size_t i = 0; i < npts; i++) {
      reader.read((char*) (data + i), sizeof(T));
      // memset(data + i * rounded_dim + dim, 0, (rounded_dim - dim) *
      // sizeof(T));
    }
    diskann::cout << " Allocated " << allocSize << "bytes and copied data "
                  << std::endl;
  ...
  template<typename T>
  inline void load_range_bin(const std::string& bin_file, T*& data,
                             size_t& npts, size_t offset = 0) {
    diskann::cout << "Reading bin file " << bin_file << " at offset " << offset
                  << "..." << std::flush;
    std::ifstream reader(bin_file, std::ios::binary | std::ios::ate);
    ...
    load_range_bin_impl(reader, data, npts, offset);
  }
